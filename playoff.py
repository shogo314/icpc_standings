...
def main():
    IN = Path().joinpath("html", "2026 ICPC Asia Pacific Championship.html")
    OUT = Path().joinpath("json", "standings_2026_asia_pacific.json")
    with IN.open("r") as f:
        text = f.read()
    ...
    for row in standing.find_all("div", class_="team-row"):
        ...
        for n, problem in zip(TaskInfo, problems.find_all("div", class_="team-col team-problem"), strict=True):
            ...
            if bg == "bg-solved" or bg == "bg-solved-first":
