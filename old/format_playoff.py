...
def to_dict(f: list[tuple[int, str]]) -> dict:
    ...
    while True:
        ...
        k += 3
        assert f[k][0] == 7 and f[k][1].startswith("<span")
        k += 1
        d["TeamName"] = f[k][1]
        k += 1
        assert f[k] == (7, '</span>')
        k += 4
        assert f[k][0] == 7 and f[k][1].startswith('<span class="university-name"')
        k += 1
        d["University"] = f[k][1]
        print(d, file=sys.stderr)
        k += 3
        assert f[k][0] == 7 and f[k][1].startswith("[")
        k += 6
...
def main():
    html_data = ""
    with open("html/2025 ICPC Asia Pacific Championship.html", "r") as f:
        html_data = f.read()
    formated = depth_analysis(bracket_analysis(html_data))
    d = to_dict(formated)
    import json

    with open("standings_2025_asia.json", "w") as f:
